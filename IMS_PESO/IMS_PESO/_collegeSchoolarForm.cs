...
namespace IMS_PESO
{
    public partial class _collegeSchoolarForm : Form
    {
        DBConn DB = new DBConn();
        public _collegeSchoolarForm()
        {
            try
            {
                DB.getDataSource();
        ...
        private void getContact()
        {
            ...
            string query = @"select * from colschoolar where code = '{0}'";
        ...
        private void insert()
        {
            ...
            try
            {
                ...
                string query = @"insert ignore into colschoolar
                                        (code, date ,surname, firstname, middlename, gender, dob, mother, father, address, contact, school, yearlevel, ave, status)
                                        values
                                        ((select if (count(id) <= 0, 'HSS - 1', concat('HSS - ', max(id) + 1)) code from colschoolar as code), @date, @surname, @firstname, @middlename, @gender, @dob, @mother, @father, @address, @contact, @school, @yearlevel, @ave, @status)";
