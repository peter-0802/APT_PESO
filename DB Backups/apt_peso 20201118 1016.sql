...
--
-- Temporary table structure for view `nsrp`
--
DROP TABLE IF EXISTS `nsrp`;
DROP VIEW IF EXISTS `nsrp`;
CREATE TABLE `nsrp` (
  `CODE` varchar(45),
  `NAME` varchar(138),
  `BIRHDAY` date,
  ...
  `4Ps?` varchar(45),
  `EMP. STATUS` varchar(45),
  `JOB PREF.` varchar(45),
  `EDUC. LEVEL` varchar(45),
  `SKILLS` varchar(45),
  `FROM` varchar(45)
);
...
/*!40000 ALTER TABLE `contacts2` DISABLE KEYS */;
/*!40000 ALTER TABLE `contacts2` ENABLE KEYS */;
...
INSERT INTO `kasambahay2` (`id`,`date`,`code`,`surname`,`firstname`,`middlename`,`dob`,`age`,`sex`,`civil_status`,`religion`,`birthplace`,`brgy`,`Municipality`,`province`,`email`,`cp_no`,`4ps`,`emp_status`,`job_pre`,`educ_level`,`skills`,`status`,`remarks`,`from`) VALUES 
 (12,'2020-01-03','KAS - 1','AS','ASD','ASD','2020-11-03','ASD','FEMALE','Single','Islam','ASD','DALAWINON','MAGSAYSAY','DAVAO DEL SUR','ASDA','ASD','ASD','ASD','ASD','ASD','ASD','ACTIVE','ASD','KASAMBAHAY'),
 (13,'2020-02-03','KAS - 13','ASD','ASD','ASD','2020-11-03','','FEMALE','Single','Islam','ASD','DALAWINON','MAGSAYSAY','DAVAO DEL SUR','ASD','ASD','ASD','ASD','ASD','ASD','ASD','ACTIVE','','KASAMBAHAY'),
 (15,'2020-04-03','KAS - 14','D','D','54','2020-11-03','ASD','FEMALE','Single','Islam','ASD','DALAWINON','MAGSAYSAY','DAVAO DEL SUR','ASD','ASD','ASD','ASD','ASD','ASD','ASD','ACTIVE','DSA','KASAMBAHAY'),
 (16,'2020-04-03','KAS - 16','GG','G','G','2020-11-03','G','MALE','Married','Islam','G','GLAMANG','MAGSAYSAY','DAVAO DEL SUR','G','G','G','GG','G','G','G','INACTIVE','G','KASAMBAHAY');
...
INSERT INTO `rwa` (`id`,`date`,`code`,`establishment_name`,`acronym`,`tin`,`employer_type`,`work_force`,`business_line`,`address`,`municipality`,`province`,`contact_person`,`position`,`tel`,`type`,`email`) VALUES 
 (8,'2020-03-03','RWA - 1','A','A','A','DO 174-17, Subcontractor','Medium (100-199)','2','22','2','2','2','2','2','Unregistered','2');
...
INSERT INTO `sra` (`id`,`Agency`,`sra_no`,`event_date`,`host`,`veneu`,`address_branch`,`rep_contact`,`surname`,`firstname`,`middlename`,`address`,`age`,`gender`,`position`,`jobsite`,`remarks`,`notes`) VALUES 
 ...
 (8,'agency','sra','2020-10-28','rep','event venue','branch','contact no','imboy','peter jay','encinares','NEW OPON','24','MALE','welder','davao','to be hired','notes'),
 (9,'agency','sra','2020-10-28','rep','event venue','branch','contact no','234','32','232','TACUL','23','FEMALE','23','23','23','notes');
...
INSERT INTO `todo` (`id`,`code`,`title`,`desc`,`assignee`,`flag`) VALUES 
 (16,'TASK - 1','A','A','A','PENDING'),
 (18,'TASK - 18','C - UPDATE','C - UPDATE','C - UPDATE','DONE');
...
--
-- Definition of view `nsrp`
--

DROP TABLE IF EXISTS `nsrp`;
DROP VIEW IF EXISTS `nsrp`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nsrp` AS select `contacts2`.`code` AS `CODE`,concat(`contacts2`.`surname`,', ',`contacts2`.`firstname`,' ',`contacts2`.`middlename`) AS `NAME`,`contacts2`.`dob` AS `BIRHDAY`,`contacts2`.`age` AS `AGE`,`contacts2`.`sex` AS `GENDER`,`contacts2`.`civil_status` AS `CIVIL STATUS`,`contacts2`.`religion` AS `RELIGION`,`contacts2`.`birthplace` AS `BIRTHPLACE`,concat(`contacts2`.`brgy`,', ',`contacts2`.`Municipality`,', ',`contacts2`.`province`) AS `ADDRESS`,`contacts2`.`email` AS `EMAIL`,`contacts2`.`cp_no` AS `CONTACT NO`,`contacts2`.`4ps` AS `4Ps?`,`contacts2`.`emp_status` AS `EMP. STATUS`,`contacts2`.`job_pre` AS `JOB PREF.`,`contacts2`.`educ_level` AS `EDUC. LEVEL`,`contacts2`.`skills` AS `SKILLS`,`contacts2`.`from` AS `FROM` from `contacts2`;
